defmodule PasswordlessWebApi.Veil do
  @moduledoc """
  Veil's main context
  """
  alias PasswordlessWebApi.Repo
  alias PasswordlessWebApi.Veil.{User, Request, Session}
  alias PasswordlessWebApiWeb.Veil.{Mailer, LoginEmail}
  alias Veil.Secure
  ...
  defp new_session_url(conn, unique_id) do
    ...
    cur_path = PasswordlessWebApiWeb.Router.Helpers.session_path(conn, :create, [request_id: unique_id])
    
    PasswordlessWebApiWeb.Router.Helpers.url(cur_uri) <> cur_path
  end
  ...
  def verify(%Session{} = session) do
    verify(PasswordlessWebApiWeb.Endpoint, session)
  end

  def verify(%Request{} = request) do
    verify(PasswordlessWebApiWeb.Endpoint, request)
  end
